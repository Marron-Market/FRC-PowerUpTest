...
///////////   Position 1 and 4   //////////
		...
		case POS1_4_DRIVEFORWARD:
			m_driveTrain.autonDriveStraight(.7);
			if (m_driveTrain.getEncoderLeftInches() > 228.73) {
				currentState = States.POS1_4_TURN;
			}
		break;
		
		case POS1_4_TURN:
			m_driveTrain.autonDriveTurn(.7 * m_direction);
			if (Math.abs(m_driveTrain.getHeading()) > 5) {  // 5 for test purpouse (make 90 normally)
		...
		case POS1_4_TURN_TO_SWICH:
			m_driveTrain.autonDriveTurn(.7 * m_direction);
			if (Math.abs(m_driveTrain.getHeading()) > 10) { 
